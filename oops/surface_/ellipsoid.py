...
class Ellipsoid(Surface):
    ...
    def coords_from_vector3(self, pos, obs=None, axes=2, derivs=False):
        ...
        if np.any(derivs):
            raise NotImplementedError("Ellipsoid.coords_from_vector3() " +
    ...
    def vector3_from_coords(self, coords, obs=None, derivs=False):
        ...
        if derivs:
            raise NotImplementedError("Ellipsoid.vector3_from_coords() " +
                                      " does not implement derivatives")
    ...
    def intercept(self, obs, los, derivs=False, t_guess=None):
        ...
        if derivs:
            ...
            print pos
            print dpos_dobs
            pos.insert_deriv("obs", dpos_dobs)
            pos.insert_deriv("los", dpos_dlos * los_norm)
            t.insert_deriv("obs", dt_dobs.as_row())
            t.insert_deriv("los", dt_dlos.as_row() * los_norm)
    ...
    def intercept_with_normal(self, normal, derivs=False):
        ...
        if derivs:
            raise NotImplementedError("Ellipsoid.intercept_with_normal() " +
                                      "does not implement derivatives")
    ...
    def _apply_exclusion(self, pos):
        ...
        is poorly-behaved in this region. Note that this method is also used by
        class Ellipsoid.
