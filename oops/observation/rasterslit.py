...
class RasterSlit(Observation):
    ...
    second dimension is simulated by rotation of the camera. The FOV describes
    the 1-D slit. This differs from a Slit subclass in that a single sensor is
    moving to emulate the 1-D slit. It differs from a RasterScan in that the FOV
    describes a 1-D slit and not a 2-D image.
    """
    ...
    def uvt(self, indices, remask=False):
        ...
        time = self.cadence.time_at_tstep(tstep, remask=remask)
            # tstep is 2-D so this re-masks where either axis out of range
        ...
        uv = Pair(uv_vals, time.mask)   # shared mask array
    ...
    def uv_range_at_time(self, time, remask=False):
        ...
        return Observation.uv_range_at_time_2d(self, time, remask,
                                               self._cross_slit_uv_axis,
                                               self._along_slit_uv_axis)
    ...
    def time_shift(self, dtime):
        ...
        obs = RasterSlit(self.axes, self.cadence.time_shift(dtime), self.fov,
...
class Test_RasterSlit(unittest.TestCase):
    ...
    def runTest(self):
        ...
        self.assertEqual(time, [0, 100, 190, 10, 110, 200, 220])
        ...
        self.assertEqual(time_min, [0, 100, 190,  9, 109, 199, 219])
        ...
        self.assertEqual(time, [0, 5, 10, 90, 95, 100, 100.5])
        ...
        self.assertEqual(time_min, [0, 5, 9.5, 90, 95, 99.5, 100.5])
        self.assertEqual(time_max, time_min + fast_cadence.texp)
