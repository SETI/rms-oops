...
class Snapshot(Observation):
    ...
    def ring_shadow_back_plane(self, min_range=0., max_range=sys.float_info.max):
        ...
        (surface_event, obs_mask) = self.back_plane_setup(surface, planet_surface)
    ...
    def any_part_object_in_view(self, path_id):
        ...
        uv_shape = self.fov.uv_shape.vals
        uv = np.array([(0,0), (uv_shape[0],0), (uv_shape[0],uv_shape[1]), (0,uv_shape[1])])
        uv_pair = Pair(uv)
