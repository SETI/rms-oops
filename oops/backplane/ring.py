...
def ring_radius(self, event_key, rmin=None, rmax=None):
    ...
    radius = self.backplanes[default_key]
...
def ring_longitude(self, event_key, reference='node'):
    ...
    longitude = self.backplanes[default_key]
    if reference == 'node':
        return longitude
...
def radial_mode(self, backplane_key, cycles, epoch, amp, peri0, speed,
    ...
    if backplane_key[0] == 'ring_radius':
        ...
        radius = self.backplanes[('_unmasked_radial_mode',) + backplane_key[1:]]
...
def ring_incidence_angle(self, event_key, pole='sunward', apparent=True):
    ...
    prograde = self.backplanes[prograde_key]
    if pole == 'prograde':
        return prograde
...
def ring_test_suite(bpt):
    ...
    for (planet, name) in bpt.planet_ring_pairs:
        ...
        bpt.gmtest(bp.ring_radius(name),
                   name + ' radius (km)',
                   limit= 1., radius=1)
        bpt.gmtest(bp.ring_angular_resolution(name) * DPR,
                   name + ' angular resolution (deg)',
                   limit=1.e-8, radius=1)
        bpt.gmtest(bp.ring_radius(name) * bp.ring_angular_resolution(name),
                   name + ' angular resolution (km)',
                   limit=0.1, radius=1)

        # Longitude
        bpt.gmtest(bp.ring_longitude(name, reference='aries') * DPR,
                   name + ' longitude wrt Aries (deg)',
                   method='mod360', limit=0.001, radius=1)
        bpt.gmtest(bp.ring_longitude(name, reference='node') * DPR,
                   name + ' longitude wrt node (deg)',
                   method='mod360', limit=0.001, radius=1)

        longitude = bp.ring_longitude(name, reference='obs') * DPR
        bpt.gmtest(longitude,
                   name + ' longitude wrt observer (deg)',
                   method='mod360', limit=0.001, radius=1)
        bpt.compare(longitude - bp.ring_longitude(name, reference='oha') * DPR,
                    180.,
                    name + ' longitude wrt observer minus wrt OHA (deg)',
                    method='mod360', limit=0.001, radius=1)

        longitude = bp.ring_longitude(name, reference='sun') * DPR
        bpt.gmtest(longitude,
                   name + ' longitude wrt Sun (deg)',
                   method='mod360', limit=0.001, radius=1)
        bpt.compare(longitude - bp.ring_longitude(name, reference='sha') * DPR,
                    180.,
                    name + ' longitude wrt Sun minus wrt SHA (deg)',
                    method='mod360', limit=1.e-13, radius=1)

        # Azimuth
        bpt.gmtest(bp.ring_azimuth(name, direction='obs') * DPR,
                   name + ' azimuth wrt observer (deg)',
                   method='mod360', limit=0.001, radius=1)
        bpt.gmtest(bp.ring_azimuth(name, direction='sun') * DPR,
                   name + ' azimuth wrt Sun (deg)',
                   method='mod360', limit=0.001, radius=1)

        # Longitude & azimuth tests
        longitude = bp.ring_longitude(name, reference='obs') * DPR
        azimuth = bp.ring_azimuth(name, 'obs') * DPR
        bpt.gmtest(azimuth - longitude,
                   name + ' azimuth minus longitude wrt observer (deg)',
                   method='mod360', limit=0.001, radius=1)

        longitude = bp.ring_longitude(name, reference='sun') * DPR
        azimuth = bp.ring_azimuth(name, 'sun') * DPR
        bpt.compare(azimuth - longitude, 0.,
                    name + ' azimuth minus longitude wrt Sun (deg)',
                    method='mod360', limit=0.1)

        # Sub-observer longitude
        bpt.gmtest(bp.ring_sub_observer_longitude(name, reference='aries') * DPR,
                   name + ' sub-observer longitude wrt Aries (deg)',
                   method='mod360', limit=0.001)
        bpt.gmtest(bp.ring_sub_observer_longitude(name, reference='node') * DPR,
                   name + ' sub-observer longitude wrt node (deg)',
                   method='mod360', limit=0.001)
        bpt.gmtest(bp.ring_sub_observer_longitude(name, reference='sun') * DPR,
                   name + ' sub-observer longitude wrt Sun (deg)',
                   method='mod360', limit=0.001)
        bpt.compare(bp.ring_sub_observer_longitude(name, reference='obs') * DPR,
                    0.,
                    name + ' sub-observer longitude wrt observer (deg)',
                    method='mod360')

        # Sub-solar longitude & azimuth
        bpt.gmtest(bp.ring_sub_solar_longitude(name, reference='aries') * DPR,
                   name + ' sub-solar longitude wrt Aries (deg)',
                   method='mod360', limit=0.001)
        bpt.gmtest(bp.ring_sub_solar_longitude(name, reference='node') * DPR,
                   name + ' sub-solar longitude wrt node (deg)',
                   method='mod360', limit=0.001)
        bpt.gmtest(bp.ring_sub_solar_longitude(name, reference='obs') * DPR,
                   name + ' sub-solar longitude wrt observer (deg)',
                   method='mod360', limit=0.001)
        bpt.compare(bp.ring_sub_solar_longitude(name, reference='sun') * DPR,
                    0.,
                    name + ' sub-solar longitude wrt Sun (deg)',
                    method='mod360')

        # Incidence, solar elevation
        incidence = bp.ring_center_incidence_angle(name, 'sunward') * DPR
        bpt.gmtest(incidence,
                   name + ' center incidence angle, sunward (deg)',
                   limit=0.001)
        bpt.compare(incidence - 90., 0.,
                    name + ' center incidence minus 90, sunward (deg)',
                    method='<')
        bpt.gmtest(bp.ring_center_incidence_angle(name, 'north') * DPR,
                   name + ' center incidence angle, north (deg)',
                   limit=0.001)
        bpt.gmtest(bp.ring_center_incidence_angle(name, 'observed') * DPR,
                   name + ' center incidence angle, observed (deg)',
                   limit=0.001)
        bpt.gmtest(bp.ring_center_incidence_angle(name, 'prograde') * DPR,
                   name + ' center incidence angle, prograde (deg)',
                   limit=0.001)

        sunward = bp.ring_incidence_angle(name, 'sunward') * DPR
        elevation = bp.ring_elevation(name, direction='sun', pole='sunward') * DPR
        generic = bp.incidence_angle(name) * DPR

        bpt.gmtest(sunward,
                   name + ' incidence angle, sunward (deg)',
                   limit=0.001, radius=1)
        bpt.compare(sunward - 90., 0.,
                    name + ' incidence angle minus 90, sunward (deg)',
                    method='<=')
        bpt.gmtest(elevation,
                   name + ' elevation of Sun, sunward (deg)',
                   limit=0.001, radius=1)
        bpt.compare(sunward + elevation, 90.,
        ...
        northward = bp.ring_incidence_angle(name, 'north') * DPR
        bpt.gmtest(northward,
                   name + ' incidence angle, north (deg)',
                   limit=0.001, radius=1)

        prograde = bp.ring_incidence_angle(name, 'prograde') * DPR
        if planet in ('JUPITER', 'SATURN', 'NEPTUNE'):
            bpt.compare(northward - prograde, 0.,
                        name + ' incidence angle, north minus prograde (deg)')
        elif planet == 'URANUS':
            bpt.compare(northward + prograde, 180.,
                        name + ' incidence angle, north plus prograde (deg)',
                        limit=1.e-13)

        incidence0 = bp.ring_incidence_angle(name) * DPR
        incidence1 = bp.ring_center_incidence_angle(name) * DPR
        bpt.compare(incidence0 - incidence1, 0.,
                    name + ' incidence angle, ring minus center (deg)',
                    limit=0.01)

        # Emission, observer elevation
        bpt.gmtest(bp.ring_center_emission_angle(name, 'sunward') * DPR,
                   name + ' center emission angle, sunward (deg)',
                   limit=0.001)
        bpt.gmtest(bp.ring_center_emission_angle(name, 'north') * DPR,
                   name + ' center emission angle, north (deg)',
                   limit=0.001)
        bpt.gmtest(bp.ring_center_emission_angle(name, 'prograde') * DPR,
                   name + ' center emission angle, prograde (deg)',
                   limit=0.001)

        emission = bp.ring_center_emission_angle(name, 'observed') * DPR
        bpt.gmtest(emission,
                   name + ' center emission angle, observed (deg)',
                   limit=0.001)
        bpt.compare(emission - 90., 0.,
                    name + ' center emission minus 90, observed (deg)',
                    method='<')

        emission = bp.ring_emission_angle(name, 'observed') * DPR
        elevation = bp.ring_elevation(name, direction='obs', pole='observed') * DPR
        generic = bp.emission_angle(name) * DPR
        ...
        sunward = bp.ring_emission_angle(name, 'sunward') * DPR
        bpt.compare(sunward - generic, 0.,
                    name + ' emission angle, sunward minus generic (deg)',
                    limit=1.e-13)

        northward = bp.ring_emission_angle(name, 'north') * DPR
        bpt.gmtest(northward,
                   name + ' emission angle, north (deg)',
                   limit=0.001, radius=1)

        prograde = bp.ring_emission_angle(name, 'prograde') * DPR
        if planet in ('JUPITER', 'SATURN', 'NEPTUNE'):
            bpt.compare(northward - prograde, 0.,
        ...
        emission0 = bp.ring_emission_angle(name) * DPR
        emission1 = bp.ring_center_emission_angle(name) * DPR
    ...
    for (planet, name) in bpt.planet_ring_pairs:
        ...
        bpt.gmtest(test3,
                   ...
                   limit=0.1, radius=1)
        bpt.gmtest(bp.ring_longitude(test3.key, 'node') * DPR,
                   name + ' longitude, modes 1 and 2, 70-100 kkm (deg)',
                   limit=0.001, method='mod360', radius=1)
