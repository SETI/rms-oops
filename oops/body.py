...
########################################
    ...
    def apply_ring_frame(self, epoch=None):
        ...
        if self.ring_frame_id is not None:
            ringframe = frame_.as_frame(self.ring_frame_id)
...
def define_ring(parent_name, ring_name, radii, keywords):
    ...
    body.apply_surface(shape, radii[1])
...
class Test_Body(unittest.TestCase):
    ...
    def runTest(self):
        ...
        moons = saturn.select_children(exclude="IRREGULAR", radius=1000)
        self.assertEqual(len(moons), 2)     # Titan and rings
