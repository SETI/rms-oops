    ...
    def transform_at_time(self, time, quick={}):
        ...
        if time.shape == ():
            ...
            # Case 2: omega_type = zero
                ...
                matrix = cspice.pxform(self.spice_reference_name,
                                        self.spice_frame_name,
                                        time.values)
            ...
            # Case 3: omega_type = numerical
                ...
                for j in range(len(times)):
                    mats[j] = cspice.pxform(self.spice_reference_name,
                                            self.spice_frame_name,
                                            times[j])
        ...
        # Case 2: omega_type = zero
            ...
            for i,t in np.ndenumerate(time.values):
                matrix[i] = cspice.pxform(self.spice_reference_name,
                                          self.spice_frame_name,
                                          t)
...
class Test_SpiceFrame(unittest.TestCase):
    ...
    def runTest(self):
        ...
        from oops.path_.path import Path
        from oops.path_.spicepath import SpicePath
        from oops.frame_.frame import QuickFrame
        ...
        cspice.furnsh(os.path.join(TESTDATA_PARENT_DIRECTORY,
                                   'SPICE', 'naif0009.tls'))
        cspice.furnsh(os.path.join(TESTDATA_PARENT_DIRECTORY,
                                   'SPICE', 'cas00149.tsc'))
        cspice.furnsh(os.path.join(TESTDATA_PARENT_DIRECTORY,
        ...
        times = Scalar((TDB-5., TDB-1.e20, TDB+2., TDB-9.5, TDB+7.7, TDB+1.e20))
