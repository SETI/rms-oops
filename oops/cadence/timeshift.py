...
class TimeShift(Cadence, Fittable):
    ...
    def __init__(self, arg, /, cadence):
        ...
        Parameters:
            arg (float or TimeShift): The initial time shift in seconds. A positive value
                shifts times later. Alternatively, specify another TimeShift object and
                this object will be linked to that one, meaning that the time shifts will
                always be equal.
            cadence (Cadence): The Cadence object to be shifted.
        """

        if isinstance(arg, TimeShift):
            self.link = arg
        else:
            self.dt = arg
            self.link = None
