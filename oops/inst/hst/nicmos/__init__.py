################################################################################
# oops/instrument/hst/nicmos/__init__.py
################################################################################

__all__ = ["nic1", "nic2", "nic3"]
...
def from_opened_fitsfile(hst_file, parameters={}):
    ...
    if detector == "NIC1":
        return oops.instrument.hst.nicmos.nic1.from_opened_fitsfile(
                                    hst_file, parameters)

    if detector == "NIC2":
        return oops.instrument.hst.nicmos.nic2.from_opened_fitsfile(
                                    hst_file, parameters)

    if detector == "NIC3":
        return oops.instrument.hst.nicmos.nic3.from_opened_fitsfile(
                                    hst_file, parameters)

    raise IOError("unrecognized detector in HST/NICMOS file " +
                   hst.filespec(hst_file) + ": " + detector)
...
class NICMOS(oops.instrument.hst.HST):
