...
class WFC(ACS):
    ...
    def register_frame(self, hst_file, fov, **parameters):
        """Overrides the default to include define a suffix for the name of the
        frame. Needed so that names for WFC1 and WFC2 are unique.
        """

        ccd = parameters['ccd']

        return super(WFC,self).register_frame(hst_file, fov,
                                              suffix = "_WFC" + str(ccd),
                                              **parameters)
    ...
    def from_opened_fitsfile(hst_file, **parameters):
        ...
        if 'layer' in parameters:
            ...
            derived_ccd = hst_file[given_layer].header['CCDCHIP']
        else:
            given_layer = None
            derived_ccd = None
        ...
        if 'ccd' in parameters:
            ...
            try:
                for layer in range(1,6):        # IndexError on 5
                    try:
                        test_ccd = hst_file[layer].header['CCDCHIP']
