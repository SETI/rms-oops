...
class Test_Vector_cross_2x2(unittest.TestCase):
  ...
  def runTest(self):
    ...
    self.assertEqual(cross1.shape, (20,30))
    self.assertEqual(type(cross1), Scalar)
    self.assertEqual(cross1.numer, ())
    self.assertEqual(cross1.denom, ())
    ...
    for i in range(N):
        self.assertAlmostEqual(z.d_df.values[i,0], dz_df0.values[i], delta=EPS)
        self.assertAlmostEqual(z.d_dg.values[i,0], dz_dg0.values[i], delta=EPS)
        self.assertAlmostEqual(z.d_dh.values[i,0], dz_dh0.values[i], delta=EPS)

        self.assertAlmostEqual(z.d_df.values[i,1], dz_df1.values[i], delta=EPS)
