...
- **Finding**: Ruff is configured with `line-length = 88` but mypy uses no line-length setting and the project rule says 100. **Evidence**: `pyproject.toml` `[tool.ruff]` vs `.claude/rules/python.md`. **Suggestion**: Align `line-length` across ruff, formatter, and project rules to a single value.
