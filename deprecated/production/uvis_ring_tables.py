    ...
    for root, dirs, files in os.walk(valid_path):
        for name in files:
            if ("DATA" in root) and (".LBL" in name):
                ...
                ob = cassini_uvis.from_file(fname)
                obs.append(ob)
                i += 1
...
def generate_table_for_index(file_name):
    """Create the geometry objects for all the image files in the list of files
        ...
        """
    output_buf = ''
    obs = load_uvis_observations(file_name)
    #obs = cassini_uvis.from_file(file_name)
    nObs = len(obs)
    ...
    for i in range(start, nObs):
        ...
        #try:
        geometry = generate_metadata(obs[i], grid_resolution)
        #except:
        #    continue
