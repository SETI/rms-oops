...
def iss_test_suite(filespec, info, display):
    ...
    show_info("Ring angular resolution (deg/pixel)", ring_angular_resolution *
                                                     oops.DPR)

    snapshot.insert_subfield("ring_radial_resolution", ring_radial_resolution)
    snapshot.insert_subfield("ring_angular_resolution", ring_angular_resolution)
    ...
    show_info("Saturn coarsest resolution (km/pixel)", saturn_resolution_max)
...
class Test_Cassini_ISS_Suite(unittest.TestCase):
    ...
    def runTest(self):
        ...
        if UNITTEST_PRINTING: oops.config.LOGGING.on("        ")

        filespec = "test_data/cassini/ISS/W1573721822_1.IMG"
        snapshot = iss_test_suite(filespec, UNITTEST_PRINTING, False)
