##########################################################################################
# tests/surface/test_centricellipsoid.py
##########################################################################################
...
def test_centricellipsoid():
    ...
    test1 = cept.element_mul(ellipsoid._unsquash).unit()
    test2 = perp.element_mul(ellipsoid._squash).unit()
    ...
    assert abs((cept2 - cept1).sep(perp) - Scalar.HALFPI).max() < 3.e-8
    ...
    assert (abs(cept.element_mul(ellipsoid._unsquash).norm() - ellipsoid._req).max()
            < 1.e-6)
