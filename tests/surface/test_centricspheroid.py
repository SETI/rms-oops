##########################################################################################
# tests/surface/test_centricspheroid.py
##########################################################################################
...
def test_centricspheroid():
    ...
    assert (lon - lon2).abs().max() < 1.e-15
    assert (lat - lat2).abs().max() < 1.e-11

    track2 = planet.vector3_from_coords((lon2,lat2))
    assert (track2 - track).norm() < 1.e-6
    ...
    track = planet.vector3_from_coords((lon,lat))
    diff = test - track
    assert (diff.norm()).abs() - z < 3.e-11
    assert diff.sep(planet.normal(track)).max() < 3.e-10

    (lon2, lat2, z2) = planet.coords_from_vector3(test, axes=3)
    (lon3, lat3, z3) = planet.coords_from_vector3(track, axes=3)
    assert (lon - lon2).abs().max() < 1.e-15
    assert (lat - lat2).abs().max() < 3.e-12
    assert (lon3 - lon2).abs().max() < 1.e-15
    assert (lat3 - lat2).abs().max() < 1.e-11
    assert z3.abs().max() < 1.e-10
    assert (z2 - z).abs().max() < 1.e-10
    ...
    assert abs(centric_lon - test_lon).max() < 1.e-8

    test_lon2 = planet.lon_from_centric(centric_lon)
    assert abs(test_lon2 - lon).max() < 1.e-8
    ...
    assert abs(centric_lat - test_lat).max() < 1.e-8

    test_lat2 = planet.lat_from_centric(centric_lat, lon)
    assert abs(test_lat2 - lat).max() < 1.e-8
    ...
    assert abs(graphic_lat - test_lat).max() < 1.e-8
    ...
    assert abs(test - pts).max() < 1.e-9

    assert np.all(t.mask == pts.mask)
    assert np.all(pts.mask[t.vals < 0.])

    normals = planet.normal(pts)

    pts.vals[...,2] *= REQ/RPOL
    assert abs(pts.norm() - REQ).max() < 1.e-8

    normals.vals[...,2] *= RPOL/REQ
    assert abs(normals.unit() - pts.unit()).max() < 1.e-14
    ...
    test1 = cept.element_mul(spheroid._unsquash).unit()
    test2 = perp.element_mul(spheroid._squash).unit()
    ...
    assert abs((cept2 - cept1).sep(perp) - Scalar.HALFPI).max() < 1.e-8
    ...
    assert (abs(cept.element_mul(spheroid._unsquash).norm() - spheroid._req).max()
            < 1.e-6)
    ...
    for i in range(3):
        ...
        assert abs(dperp_dpos - ref).max() < 1.e-4
    ...
    for i in range(3):
        ...
        assert abs(dcept_dpos.sep(perp) - Scalar.HALFPI).max() < 1.e-5

        ref = Vector3(cept.d_dpos.vals[...,i], cept.d_dpos.mask)
        assert abs(dcept_dpos - ref).max() < 1.e-5
    ...
    assert abs(lat - test_lat).max() < 1.e-8
