##########################################################################################
# tests/surface/test_spheroid.py
##########################################################################################
...
def test_spheroid():
    ...
    diff = test2 - test3
    assert diff.norm() - z < 3.e-11
    assert diff.sep(planet.normal(test3)).max() < 1.e-10
...
def _points_off_the_surface(surface: Spheroid) -> Vector3:
...
def test_intercept_normal_to_recovers_from_a_bad_guess() -> None:
    ...
    surface = Spheroid('SSB', 'J2000', (1000., 800.))
