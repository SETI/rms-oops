...
def test_barrelfov():
    ...
    assert fov.max_inversion_error() < 3.e-13
    ...
    DEL = 1.e-6
    assert abs(uv.d_dt.vals         - duv_dt.vals).max() <= DEL
    ...
    assert fov.max_inversion_error() < 3.e-14
    assert fov_fast.max_inversion_error() < 0.3
...
def test_barrelfov_masked():
    ...
    for shape in ((7,), (100,), (5,4)):
        ...
        assert type(uv) is Pair
        assert uv.shape == shape
