...
def shrink_int_array(values):
    ...
    if amin >= -128 and amax < 128:
        offset = 0
        compressed = values.astype('int8')

    elif amin >= 0 and amax < 256:
        offset = 0
        compressed = values.astype('uint8')

    elif diff < 256:
        offset = amin
        compressed = (values - amin).astype('uint8')

    elif amin >= -2**15 and amax < 2**15 :
        offset = 0
        compressed = values.astype('int16')
        ...
        offset = amin
        compressed = (values - amin).astype('uint16')
        ...
        offset = amin
        compressed = (values - amin).astype('uint32')
...
def encode_float_array(values, b64_savings, npz_savings, single=False,
    ...
    if text_bytes < b64_bytes + b64_savings:
        if storage_dtype != 'float64':
            storage_dtype = 'float32'
            attr_list.append(('storage_dtype', storage_dtype))
...
class test_packrat_arrays(unittest.TestCase):
  ...
  def runTest(self):
    ...
    self.assertEqual(attr['storage_dtype'], 'int8')
    ...
    self.assertEqual(attr['encoding'], 'npz')
    self.assertEqual(attr['storage_dtype'], 'int16')

    attr = test_int_array(np.arange(10000) - 50000, 1000, 10000)
    self.assertEqual(attr['encoding'], 'npz')
    self.assertEqual(attr['storage_dtype'], 'uint16')
    self.assertEqual(eval(attr['offset']), -50000)

    attr = test_int_array(np.arange(10000) * 50000, 1000, 10000)
    self.assertEqual(attr['encoding'], 'npz')
    self.assertEqual(attr['storage_dtype'], 'int32')

    attr = test_int_array(np.arange(65535) * 65535, 1000, 10000)
    self.assertEqual(attr['encoding'], 'npz')
    self.assertEqual(attr['storage_dtype'], 'uint32')
    ...
    self.assertEqual(attr['encoding'], 'base64')
    self.assertEqual(attr['storage_dtype'], 'float64')
    ...
    self.assertEqual(attr['encoding'], 'text')
    self.assertEqual(attr['storage_dtype'], 'float32')
    ...
    self.assertEqual(attr['storage_dtype'], 'uint8')
