...
################################################################################
def get_error_buffer_size(snapshot):
    error_buffer = 60
...
def backplane_for_observation(obs, resolution):
    # deal with possible pointing errors - up to 3 pixels in any
    # direction for WAC and 30 pixels for NAC
    error_buffer = get_error_buffer_size(obs)
    limit = obs.fov.uv_shape + oops.Pair(np.array([error_buffer,
...
def generate_bp_metadata(bp, obs, ndx):
    ...
    body_list = bodies_for_observation(get_observation_id(obs))
    """if TIME_DEBUG:
        now = datetime.datetime.now()
        duration = now - then
    ...
    for b_name in body_list:
        ...
        if body_name == 'saturn' or body_name == 'titan':
            #onto the Limb
            limb_body = body_name + ":limb"
            body_intercepted = bp.where_intercepted(limb_body)
            limb_mask = ~body_intercepted.vals
...
################################################################################
def generate_process_metadata(obs, resolution):
...
def generate_geometries_for_index(file_name):
    ...
    for i in range(start, nObs):
        ...
        some_geometries = generate_process_metadata(ob, grid_resolution)
