...
class QuickPath(Path):
    ...
    def extend(self, tmin, tmax):
        ...
        Parameters:
            tmin (float): The new earliest time to tabulate in this QuickPath.
            tmax (float): The new latest time to tabulate in this QuickPath.
        """
    ...
    def for_path(path, time, *, quick=None):
        """A new QuickPath that approximates the given path within given time limits.
        ...
        Parameters:
            ...
            time (Scalar or tuple): The set of times at which this path is to be
        ...
        Returns:
            Path: A QuickPath approximation to the given `path` if the overhead required
            to create it makes this effort worthwhile. Otherwise, the given `path` is
            returned.
