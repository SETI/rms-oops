...
from oops._exceptions       import OopsKeyError
...
def spice_shape(spice_id, frame=None, default_radii=None):
    ...
    Raises:
        ...
        OopsKeyError: If the body's radius values are missing from the SPICE kernel pool
            and `default_radii` is not provided.
    ...
    try:
        ...
        if default_radii is None:
            raise OopsKeyError('radii are not available for SPICE body '
