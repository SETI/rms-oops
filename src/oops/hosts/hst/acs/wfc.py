##########################################################################################
# oops/hosts/hst/acs/wfc.py
##########################################################################################

import astropy.io.fits as pyfits

from .                import ACS
from oops._exceptions import OopsValueError

from filecache import FCPath

##########################################################################################
# Classless method
##########################################################################################

def from_file(filespec, **parameters):
    """An Observation object based on a given data file generated by HST/ACS/WFC.

    Parameters:
        filespec (str | pathlib.Path | FCPath | astropy.io.fits.HDUList): The path to
            the HST/ACS/WFC FITS file, or the file already opened.
        **parameters (Any): The options recognized by :func:`~oops.hosts.hst.from_file`,
            plus:

            * `ccd` (int): 1 for WFC1; 2 for WFC2.
            * `layer` (int): 1 for the first image in FITS file, 2 for the second.

            Either layer or ccd must be specified. If both are specified, they must be
            compatible.

    Returns:
        Snapshot: The observation, with subfields `layer` and `chip` identifying the
        image.

    Raises:
        IOError: If the file was not generated by HST/ACS/WFC.
        ValueError: If neither `ccd` nor `layer` is given, or the two are incompatible.
    """

    # Open the file if necessary
    if isinstance(filespec, pyfits.HDUList):
        hdulist = filespec
    else:
        filespec = FCPath(filespec)
        local_path = filespec.retrieve()
        hdulist = pyfits.open(local_path)

    # Make an instance of the WFC class
    this = WFC()

    # Confirm that the telescope is HST
    if this.telescope_name(hdulist) != 'HST':
        raise IOError(f'not an HST file: {filespec}')

    # Confirm that the instrument is ACS
    if this.instrument_name(hdulist) != 'ACS':
        raise IOError(f'not an HST/ACS file: {filespec}')

    # Confirm that the detector is WFC
    if this.detector_name(hdulist) != 'WFC':
        raise IOError(f'not an HST/ACS/WFC file: {filespec}')

    return WFC.from_hdulist(hdulist, **parameters)

##########################################################################################
# Class WFC
##########################################################################################

IDC_DICT = None

GENERAL_SYN_FILES = ['OTA/hst_ota_???_syn.fits', 'ACS/acs_wfc_im123_???_syn.fits']

CCD_SYN_FILE_PARTS    = ['ACS/acs_wfc_ccd', '_???_syn.fits']
FILTER_SYN_FILE_PARTS = ['ACS/acs_', '_???_syn.fits']


class WFC(ACS):
    """The unique detector properties of the ACS/WFC.

    Other properties are inherited from higher levels in the class hierarcy. All functions
    are static so this class requires no instances.
    """

    def define_fov(self, hdulist, **parameters):
        """An FOV object defining the field of view of the given image file.

        Parameters:
            hdulist (astropy.io.fits.HDUList): The opened FITS file.
            **parameters (Any): Additional parameters. The key `layer` (int) is
                required; it selects the HDU whose header identifies the CCD chip.

        Returns:
            FOV: The field of view, as constructed by
            :meth:`~oops.hosts.hst.HST.construct_fov`.
        """

        global IDC_DICT

        # Load the dictionary of IDC parameters if necessary
        if IDC_DICT is None:
            IDC_DICT = self.load_idc_dict(hdulist, ('DETCHIP', 'FILTER1', 'FILTER2'))

        # Determine the layer of the FITS file to read
        layer = parameters['layer']

        # Define the key into the dictionary
        idc_key = (hdulist[layer].header['CCDCHIP'], hdulist[0].header['FILTER1'],
                                                     hdulist[0].header['FILTER2'])

        # Use the default function defined at the HST level for completing the
        # definition of the FOV
        return self.construct_fov(IDC_DICT[idc_key], hdulist)

    def select_syn_files(self, hdulist, **parameters):
        """The SYN files defining the throughput of this filter combination.

        The profiles in these files are multiplied together to obtain the throughput of
        the given instrument, detector and filter combination.

        Parameters:
            hdulist (astropy.io.fits.HDUList): The opened FITS file.
            **parameters (Any): Additional parameters. The key `layer` (int) is
                required; it selects the HDU whose header identifies the CCD chip.

        Returns:
            list[str]: The paths of the SYN files relative to the SYN root directory,
            each containing a glob pattern in place of the version number.
        """

        global GENERAL_SYN_FILES, CCD_SYN_FILE_PARTS, FILTER_SYN_FILE_PARTS

        # Copy all the standard file names
        syn_filenames = []
        for filename in GENERAL_SYN_FILES:
            syn_filenames.append(filename)

        # Add the filter file names
        for filter_name in (hdulist[0].header['FILTER1'], hdulist[0].header['FILTER2']):

            if filter_name[0:3] == 'POL':
                if filter_name[-2:] == 'UV':
                    filter_name = 'POL_UV'
                else:
                    filter_name = 'POL_V'

            if filter_name[0:5] != 'CLEAR':
                syn_filenames.append(FILTER_SYN_FILE_PARTS[0] + filter_name.lower() +
                                     FILTER_SYN_FILE_PARTS[1])

        # Determine the layer of the FITS file to read
        layer = parameters['layer']

        # Add the CCD file name
        syn_filenames.append(CCD_SYN_FILE_PARTS[0] +
                             str(hdulist[layer].header['CCDCHIP']) +
                             CCD_SYN_FILE_PARTS[1])

        return syn_filenames

    def register_frame(self, hdulist, fov, **parameters):
        """Override the default to define a suffix for the name of the frame.

        Needed so that names for WFC1 and WFC2 are unique.

        Parameters:
            hdulist (astropy.io.fits.HDUList): The opened FITS file.
            fov (FOV): The field of view of the observation.
            **parameters (Any): The options recognized by
                :meth:`~oops.hosts.hst.HST.register_frame`. The key `ccd` (int) is
                required; it supplies the suffix.

        Returns:
            str | Frame | None: The ID of the registered frame, or the frame of the
            reference observation when the POS TARG values match, or None if the header
            does not describe a celestial (RA, DEC) coordinate system.
        """

        ccd = parameters['ccd']

        return super(WFC,self).register_frame(hdulist, fov, suffix='_WFC' + str(ccd),
                                              **parameters)

    @staticmethod
    def from_hdulist(hdulist, **parameters):
        """An Observation object based on an HST data file generated by HST/ACS/WFC.

        Parameters:
            hdulist (astropy.io.fits.HDUList): The opened FITS file.
            **parameters (Any): The options recognized by
                :func:`~oops.hosts.hst.acs.wfc.from_file`.

        Returns:
            Snapshot: The observation, with subfields `layer` and `chip` identifying the
            image.

        Raises:
            OopsValueError: If neither `ccd` nor `layer` is given, or the two are
                incompatible.
        """

        # Make an instance of the WFC class
        this = WFC()

        # Figure out the layer and CCD; make sure they are compatible
        if 'layer' in parameters:
            given_layer = parameters['layer']
            derived_ccd = hdulist[given_layer].header['CCDCHIP']
        else:
            given_layer = None
            derived_ccd = None

        if 'ccd' in parameters:
            given_ccd = parameters['ccd']

            try:
                for layer in range(1,6):        # IndexError on 5
                    try:
                        test_ccd = hdulist[layer].header['CCDCHIP']
                        if test_ccd == given_ccd:
                            derived_layer = layer
                            break
                    except KeyError:
                        pass

            except IndexError:
                raise OopsValueError(f'CCD {given_ccd} not found: ' +
                                     this.filespec(hdulist))

        else:
            given_ccd = None
            derived_layer = None

        ccd = given_ccd or derived_ccd
        layer = given_layer or derived_layer

        if ccd is None or layer is None:
            raise OopsValueError('layer and ccd parameters are undefined: ' +
                                 this.filespec(hdulist))

        if derived_ccd not in (None, ccd) or \
           derived_layer not in (None, layer):
            raise OopsValueError('layer and ccd parameters are incompatible: ' +
                                 this.filespec(hdulist))

        parameters = parameters.copy()
        parameters['ccd'] = ccd
        parameters['layer'] = layer

        obs = this.construct_snapshot(hdulist, **parameters)

        # Insert subfields common to all images
        obs.insert_subfield('layer', layer)
        obs.insert_subfield('chip', ccd)

        return obs

##########################################################################################
