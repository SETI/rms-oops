...
def from_file(filespec, **options):
    ...
    Parameters:
        filespec (str | pathlib.Path | FCPath): File path to the FITS file.
        **options (Any): The keyword options listed below.
    ...
    Keyword Arguments:
        ...
        navigation (tuple | list | numpy.ndarray | bool, optional): An optional
        ...
        offset (PairLike, optional): An optional pair of coordinate offsets (du, dv) in
            units of pixels to apply to the FITS-derived geometry in order to align with
            the actual image geometry. This is an alternative to specifying the navigation
            angles.
        ...
        cal_file (bool | str, optional): If True or if this is a file path, this indicates
            that the observation should inherit the geometry and calibration of an
            associated "_cal.fits" file. Default is False. If provided, the returned
            observation has a subfield "cal" containing the calibrated observation.
        cal_factor (float, optional): The scale factor to pre-scale raw values of DN per
            second before inheriting the calibration objects of the cal_file. If not
            provided along with cal_file, the value will be derived by linear regression,
            comparing the raw and calibrated data.
        baseline (float, optional): The baseline value in raw DN to subtract from any
            image layers that contain the dark current. This is used for the pre-scaling
            needed to inherit calibrations from the cal_file. If not provided along with
            cal_file, the value will be derived by linear regression, comparing the raw
            and calibrated data.
        diffs (bool, optional): True (the default) to replace the data in each group after
            the first in the 4-D data array by a successive difference from the previous
            group.
        per_second (bool, optional): True to divide all array values by the associated
            exposure time, yielding units of DN/s. If False (the default), the data arrays
            contain the raw integer DNs.
        groups (bool, optional): True (the default) to in include a subfield "groups" in
            the returned object. This is an array of shape (integrations, groups), in
            which each element is an individual TimedImage describing one individual raw
    ...
    Raises:
        OopsValueError: If the file is not an "_uncal.fits" file.
        IOError: If the file was not generated by JWST/NIRCam.
    """
    ...
    if not filespec.name.lower().endswith('_uncal.fits'):
        raise oops.OopsValueError('not an _uncal file: ' + filespec)
...
class Uncal(NIRCam):
    ...
    def from_hdulist(hdulist, **options):
        ...
        Parameters:
            hdulist (astropy.io.fits.HDUList): The HDU list of the FITS file.
            **options (Any): The options accepted by
                :func:`~oops.hosts.jwst.nircam.uncal.from_file`.
    ...
    def fit_to_calibrated(raw_hdulist, cal_hdulist, diff_texp, cal_factor=0.):
        ...
        Parameters:
            raw_hdulist (astropy.io.fits.HDUList): The HDU list of the "_uncal.fits" file.
            cal_hdulist (astropy.io.fits.HDUList): The HDU list of the associated
                "_cal.fits" file.
            diff_texp (numpy.ndarray): The exposure time in seconds of each group, as a
                successive difference from the previous group.
            cal_factor (float, optional): The slope, if already known; zero to solve for
                it.

        Returns:
            tuple[float, float]: The slope and the baseline.
        """
