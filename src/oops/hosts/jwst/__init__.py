...
def from_file(filespec, **options):
    """An Observation based on a data file generated by JWST.
    ...
    Keyword Arguments:
        ...
        parallel (Observation, optional): An optional Observation object defining the
            ...
            observation to the detector of this observation.
    ...
    Returns:
        Observation: The observation described by the file.

    Raises:
        IOError: If the file was not generated by JWST or its instrument is not
            supported.
    """
...
class JWST(object):
    """The functions and properties unique to the James Webb Space Telescope.
    ...
    def filespec(self, hdulist, **options):
        ...
        Parameters:
            ...
            **options (Any): Accepted and ignored.

        Returns:
            str: The path of the file from which the HDU list was read.
        """
    ...
    def basename(self, hdulist, **options):
        ...
        Returns:
            str: The base name of the file from which the HDU list was read.
        """
    ...
    def instrument_name(self, hdulist, **options):
        ...
        Returns:
            str: The instrument name in its standard capitalization.
        """
    ...
    def header_subfields(self, hdulist, **options):
        ...
        Parameters:
            hdulist (astropy.io.fits.HDUList): The HDU list of the FITS file.
            **options (Any): The options of :func:`~oops.hosts.jwst.from_file`. The data
                arrays are included if `data` is True, or if `data` is unspecified and
                `astrometry` is not True; `target` overrides the target name.

        Returns:
            dict[str, Any]: The subfields "this", "filepath", "basename", "headers",
            "telescope", "instrument", "detector", "filter", "subarray", "target" and
            "dither", plus "data", "error" and "quality" when the data arrays are
            included.
        """
    ...
    def check_options(self, options):
        ...
        Parameters:
            options (dict[str, Any]): The options of :func:`~oops.hosts.jwst.from_file`.

        Returns:
            dict[str, Any]: The same dictionary, with defaults filled in for "data",
    ...
    def time_limits(self, hdulist, **options):
        """The overall start and end times of the observation, in seconds TDB.
    ...
    def row_cadence(self, hdulist, **options):
        ...
        Returns:
            Cadence: A Metronome with one step per row, reversed if the slow axis of the
            detector readout is negative.
        """
    ...
    def jwst_path(self, hdulist, **options):
        ...
        Parameters:
            hdulist (astropy.io.fits.HDUList): The HDU list of the FITS file.
            **options (Any): The options of :func:`~oops.hosts.jwst.from_file`;
                `path_suffix` is appended to the file basename to form the path ID.

        Returns:
            LinearPath: The path of JWST relative to the solar system barycenter.
        """
    ...
    def target_name(self, hdulist, **options):
        ...
        Parameters:
            hdulist (astropy.io.fits.HDUList): The HDU list of the FITS file.
            **options (Any): The options of :func:`~oops.hosts.jwst.from_file`; `target`,
                if specified, overrides the TARGNAME keyword of the header.

        Returns:
            str: The name of the target body.
        """
    ...
    def tracker_frame(self, hdulist, fov, path, **options):
        ...
        Parameters:
            hdulist (astropy.io.fits.HDUList): The HDU list of the FITS file.
            fov (WCSFOV): The FOV of the observation, which defines the pointing.
            path (Path): The path of JWST.
            **options (Any): The options of :func:`~oops.hosts.jwst.from_file`;
                `frame_suffix` is appended to the file basename to form the frame ID and
                `target` overrides the target name.

        Returns:
            TrackerFrame: The frame tracking the target body.
        """
    ...
    def offset_frame(self, hdulist, reference, **options):
        """A PosTargFrame for the observation.

        It is based on a fixed offset from the frame of another observation.

        Parameters:
            hdulist (astropy.io.fits.HDUList): The HDU list of the FITS file.
            reference (Observation): The JWST observation from whose frame the offset is
                measured.
            **options (Any): The options of :func:`~oops.hosts.jwst.from_file`;
                `frame_suffix` is appended to the file basename to form the frame ID.

        Returns:
            PosTargFrame: The frame offset from that of the reference observation.
        """
    ...
    def instrument_frame(self, hdulist, fov, path, **options):
        """The instrument frame of the observation.

        This is either a TrackerFrame or a PosTargFrame, depending on the presence of the
        options `reference`, `navigation`, `offset`, etc.

        Parameters:
            hdulist (astropy.io.fits.HDUList): The HDU list of the FITS file.
            fov (WCSFOV): The FOV of the observation.
            path (Path): The path of JWST.
            **options (Any): The options of :func:`~oops.hosts.jwst.from_file`. A
                PosTargFrame is returned if `reference` is specified and a TrackerFrame
                otherwise; either is wrapped in a Navigation frame if `navigation` or
                `offset` is specified, with `origin` and `parallel` interpreting the
                offset.

        Returns:
            Frame: The instrument frame of the observation.
        """
    ...
    def iof_factor(self, hdulist, path, **options):
        ...
        Parameters:
            ...
            path (Path): The path of JWST.
            **options (Any): The options of :func:`~oops.hosts.jwst.from_file`;
                `solar_model` names the solar spectrum model, "STIS_Rieke" by default, and
                `target` overrides the target name.

        Returns:
            float: The factor converting MJy per steradian to I/F, averaged over the
            filter bandpass and scaled to the distance of the target from the Sun.

        Raises:
            IOError: If the calibration unit of the file is missing or is not "MJy/sr".
        """
    ...
    def from_hdulist(hdulist, **options):
        """An Observation based on the HDUlist from a JWST FITS data file.
