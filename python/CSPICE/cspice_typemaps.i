...
//      (Type IN_ARRAY1[ANY])
    ...
    if (array->dimensions[0] != $1_dim0) {                      // ARRAY_dim0
        PyErr_Format(PyExc_ValueError,
            ...
            $1_dim0, (int) array->dimensions[0]);               // ARRAY_dim0
        SWIG_fail;
    }
...
//      (int DIM1, Type IN_ARRAY1[ANY])
    ...
    if (array->dimensions[0] != $2_dim0) {                      // ARRAY_dim0
        PyErr_Format(PyExc_ValueError,
            "Array of size %d was expected; size %d was given",
            $2_dim0, (int) array->dimensions[0]);               // ARRAY_dim0
        SWIG_fail;
    }
    ...
    if (array->dimensions[0] != $1_dim0 ||                      // ARRAY_dim0
        ...
        PyErr_Format(PyExc_ValueError,
            ...
            $1_dim0, $1_dim1, (int) array->dimensions[0],
                              (int) array->dimensions[1]);      // ARRAY_dimX
        SWIG_fail;
    }
...
//      (Type IN_ARRAY2[ANY][ANY], int DIM1, int DIM2)
    ...
    if (array->dimensions[0] != $1_dim0 ||                      // ARRAY_dim0
        ...
        PyErr_Format(PyExc_ValueError,
            "Array of size (%d,%d) was expected; size (%d,%d) was given",
            $1_dim0, $1_dim1, (int) array->dimensions[0],
                              (int) array->dimensions[1]);      // ARRAY _dim0,1
        SWIG_fail;
    }
...
%typemap(in) (char *IN_STRING) {
    ...
    buffer = (char *) PyMem_Malloc((len+1) * sizeof(char));
    if (!buffer) {
        PyErr_SetString(PyExc_MemoryError, "Failed to allocate memory");
        SWIG_fail;
    }
...
//      (... char OUT_STRING[ANY] ...)
    ...
    PyObject* obj = PyString_FromString((char *) buffer$argnum);
