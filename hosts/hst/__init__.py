...
def from_file(filespec, **parameters):
    ...
    the Sun to the target body for calibration purposes.
    """
...
class HST(object):
    ...
    def pos_targ(self, hst_file, **parameters):
        """A tuple containing the POS TARG values (x,y) used for this
    ...
    def register_tracker_frame(self, hst_file, fov, suffix="", **parameters):
        """Construct a Tracker frame for the observation. This frame ensures
        that the target object stays at the same location on the CCD for the
        ...
        tracker = oops.frame.TrackerFrame(cmatrix, target_body.path,
    ...
    def register_postarg_frame(self, hst_file, fov, suffix="", **parameters):
        ...
        frame = oops.frame.PosTargFrame(xpos_corrected, ypos_corrected,
    ...
    def iof_calibration(self, hst_file, fov, extended=True, **parameters):
        """A Calibration object suitable for integrating the I/F of point
        objects in an HST image.
    ...
    def target_body(self, hst_file, **parameters):
        """The body object defining the image target. It is based on educated
        guesses from the target name used by the PI.
        """
...
#                 raise ValueError('Target body not identified: ' + targname)
    ...
    def set_idc_path(self, idc_path):
        """Define the directory path to the IDC files. It must be called before
    ...
    def idc_filespec(self, hst_file):
        """The full directory path and file name of an IDC file, given the
        associated FITS file object.
        """
    ...
    def construct_idc_fov(self, fov_dict, platescale=1.):
        """The FOV object associated with the full field of view of an HST
        instrument, based on a dictionary of associated IDC parameter values.
    ...
    def construct_fov(self, fov_dict, hst_file, platescale=1.):
        """The FOV object associated with an HST instrument, allowing for
    ...
    def load_syn_throughput(self, hst_file, **parameters):
        """A Tabulation of throughput vs. wavelength in Angstroms for
    ...
    def compare_bandwidth_rms(self, hst_file):
        """A tuple containing the RMS bandwidth as derived from the SYN file and
        that extracted directly from the file header.
